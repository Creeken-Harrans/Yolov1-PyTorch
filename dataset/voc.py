...
def _require_child(element, tag):
...
def _require_text(element, tag):
    child = _require_child(element, tag)
    if child.text is None:
        raise ValueError(f'Missing text for XML tag: {tag}')
    return child.text
...
def load_images_and_anns(im_sets, label2idx, ann_fname, split):
    ...
    for im_set in im_sets:
        ...
        for im_name in im_names:
            ...
            width = int(_require_text(size, 'width'))
            height = int(_require_text(size, 'height'))
            ...
            for obj in ann_info.findall('object'):
                det = {}
                label = label2idx[_require_text(obj, 'name')]
                difficult = int(_require_text(obj, 'difficult'))
                bbox_info = _require_child(obj, 'bndbox')
                bbox = [
                    int(float(_require_text(bbox_info, 'xmin'))) - 1,
                    int(float(_require_text(bbox_info, 'ymin'))) - 1,
                    int(float(_require_text(bbox_info, 'xmax'))) - 1,
                    int(float(_require_text(bbox_info, 'ymax'))) - 1,
                ]
                det['label'] = label
                det['bbox'] = bbox
...
class VOCDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if im is None:
            raise FileNotFoundError(f"Failed to read image: {im_info['filename']}")
